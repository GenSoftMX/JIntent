...
class ValidateCounterConfigUseCase extends JSyncUseCase<CounterConfig, bool> {
  ...
  Either<Exception, CounterConfig> _validateStepIsPositive(CounterConfig input) {
    if (input.step <= 0) {
      return Left(
        Exception('Step must be positive, got: ${input.step}'),
      );
    }
    return Right(input);
  }
  ...
  Either<Exception, CounterConfig> _validateMinLessThanMax(CounterConfig input) {
  ...
  Either<Exception, CounterConfig> _validateInitialInRange(CounterConfig input) {

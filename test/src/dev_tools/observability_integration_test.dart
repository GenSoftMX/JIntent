...
class TestController extends JController<TestState> {
  TestController(super.initialState);
...
class TestIntent extends JIntent<TestState> with JIntentHelpers {
  ...
  Future<void> onInvoke() async {
    // Use existing correlation context if available, otherwise create new one
    Future<void> runLogic() async {
    ...
    // Only create new correlation context if one doesn't exist
    if (CorrelationContext.current != null) {
      await runLogic();
    } else {
      await CorrelationContext.runWithCorrelation(runLogic);
    }
  }
}
...
void main() {
  group('Observability Integration', () {
    ...
    setUp(() {
      ...
      // Create controller
      controller = TestController(const TestState());
    });
    ...
    test('complete observability flow - success case', () async {
      await controller.intent(TestIntent(logger));

      // Verify logs were captured
      expect(capturedLogs.isNotEmpty, true);

      // Parse and verify log structure (filter out non-JSON debug messages)
      final logs =
          capturedLogs
              .where((log) => log.trim().startsWith('{'))
      ...
      final completionLogs = logs.where(
        (l) => l['message'].contains('completed'),
      );
      ...
      final logsWithCorrelation = logs.where(
        (l) => l['context']?['correlationId'] != null,
      );
      ...
      final timerMetrics = JMetrics.getMetricsByName(
        'test.intent.duration.duration',
      );
    ...
    test('complete observability flow - failure case', () async {
      try {
        await controller.intent(TestIntent(logger, shouldFail: true));
        fail('Should have thrown exception');
      } catch (e) {
        expect(e.toString(), contains('Intentional failure'));
      }

      // Verify error log was captured (filter out non-JSON debug messages)
      final logs =
          capturedLogs
              .where((log) => log.trim().startsWith('{'))
              .map((log) => jsonDecode(log) as Map<String, dynamic>)
      ...
      final failedTimers = timerMetrics.where(
        (m) => m.tags['status'] == 'failed',
      );
    ...
    test('correlation ID propagates through async operations', () async {
      ...
      // Verify correlation ID was in logs (filter out non-JSON debug messages)
      final logs =
          capturedLogs
              .where((log) => log.trim().startsWith('{'))
              .map((log) => jsonDecode(log) as Map<String, dynamic>)
      ...
      final logsWithCorrelation = logs.where(
        (l) => l['context']?['correlationId'] == capturedCorrelationId,
      );
      expect(logsWithCorrelation.isNotEmpty, true);
    });

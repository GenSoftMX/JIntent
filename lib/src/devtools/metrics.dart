...
class JMetrics {
  ...
  static void recordHistogram(String name, num value, {Map<String, String>? tags}) {
    if (!_enabled) return;
  ...
  static void attachToObserver() {
    ...
    JObserver.onIntentDispatched = (intent) {
      incrementCounter('intent.dispatched', tags: {
        'type': intent.runtimeType.toString(),
      });
      
      // Start timing intent execution
      final timerId = startTimer('intent.execution', tags: {
        'type': intent.runtimeType.toString(),
      });
    ...
    JObserver.onStateChanged = (prev, next, origin) {
      incrementCounter('state.changed', tags: {
    ...
    JObserver.onEffectEmitted = (effect) {
      incrementCounter('effect.emitted', tags: {
